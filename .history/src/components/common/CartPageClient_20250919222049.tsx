...
export default function CartPageClient() {
  ...
  const { cart, handleRemoveItem, setCart, formatPrice } = useAppContext();

  useEffect(() => {
    if (!cart || !cart.data || cart.data.products === undefined) {
      setPageLoading(true);
    } else {
    ...
    <div className="px-4 py-12 md:px-8 lg:px-16 xl:px-32 2xl:px-40 pt-24 ">
      {pageLoading ? (
        <div className="flex flex-col items-center justify-center mt-20">
          <div className="w-20 h-20 border-4 border-[#F35C7A] border-t-transparent rounded-full animate-spin"></div>
