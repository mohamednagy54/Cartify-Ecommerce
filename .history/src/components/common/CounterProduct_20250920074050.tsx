...
export default function CounterProduct({
  ...
  return (
    <div className="flex items-center gap-2 bg-gray-100 rounded-3xl px-4 py-2">
      ...
      {loading ? (
        <div className={`w-5 h-5 border-2 border-gray-400 border-t-transparent rounded-full animate-spin ${isHero ? "bg-black/40 hover:bg-[#F35C7A] cursor-pointer backdrop-blur-sm border border-white/30 text-white" : ""}`}></div>
      ) : (
